...
(function(){
	var Storage = Function.create({
		...
		read: function(url, options){
			...
			return this.createGetPromise(request);
		},
		...
		write: function(url, body, options){
			...
			return this.createSetPromise(request);
		}
	});
	...
	var store = {
		...
		createHttpResponse: function(request){
			return this.env.createResponse(request);
		},

		createFakeResponse: function(request){

		},

		resolveRequest: function(client){
			return client.then(function(response){
				if( response.readyState === 'closed' ) return response;
		...
		resolveHttpRequest: function(request){
			var client = new HttpClient(request, this.createHttpResponse);
			return this.resolveRequest(client);
		},

		resolveFakeRequest: function(request){
			var client = new HttpClient(request, this.createFakeResponse);
			return this.resolveRequest(client);
		},
		...
		createHttpStorage: function(){
			return {
				createGetPromise: function(request){
					return this.store.resolveHttpRequest(request);
				},
