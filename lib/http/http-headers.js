...
(function(jsenv){
	...
	var HeadersIterator = Function.extend(Iterator, {
		constructor: function(headers, iterationType){
			iterationType = iterationType || 'key+value';

			if( iterationType === 'key' ) return new Iterator(headers, true);
			if( iterationType === 'key+value' ) return new Iterator(headers);
