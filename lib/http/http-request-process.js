(function(){
	...
	var ProcessHttpRequest = {
		...
		connect: function(){
			var request, response, url, isHttps, connection, headers, options;
			...
			connection.on('response', function(incomingMessage){
				request.opened(incomingMessage.statusCode, incomingMessage.headers, new DuplexStream(incomingMessage));
			});
