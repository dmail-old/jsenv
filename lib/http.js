(function(){
	...
	var HttpClient = Function.create({
		...
		constructor: function(request, listener){
			...
			this.request.onopen = function(){
				this.state = 'opened';
				if( false === this.hasOwnProperty('isWaitingOpen') ){ // only the first open
					this.emit('open');
				}
				this.handleResponse();
			}.bind(this);
		...
		handleResponse: function(){
			var pipes = this.pipes.reverse(), i = pipes.length, result;
	...
	var http = {
		...
		extendRequest: function(promiseFactory){
			return Function.extend(this.FakeRequest, {
