...
(function(jsenv){
	...
	var handler = {
		...
		createParserNotFoundError: function(mediaType){
			var error = new Error(mediaType + ' has no associated parser');
			return error;
		},

		get: function(mediaType){
			var parser, parsers = this.parsers, i = 0, j = parsers.length;

			for(;i<j;i++){
				parser = parsers[i];
				if( parser.type === mediaType ){
					break;
				}
				else{
					parser = null;
	...
	function getMediaType(module){
		...
		return module.meta.response.headers['content-type'] || mediaTypes[module.extname.slice(1)];
	}

	function getParser(module){
		return handler.get(getMediaType(module));
	}
	...
	var jsenvLoader = Function.extend(es6Loader, {
		...
		fetch: function(module){
			...
			return module.storage.fetch().then(function(body){
				module.parser = getParser(module);
				return body;
			});
		},

		collectDependencies: function(module){
			return module.parser.collectDependencies();
		},

		parse: function(module){
			return module.parser.parse();
		},

		execute: function(module){
			return module.parser.execute();
		}
	});
